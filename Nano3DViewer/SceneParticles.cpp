...
void SceneParticles::init()
{
    prog.compileShader("shaders/particles_vert.glsl", "shaders/particles_frag.glsl");
    flatProg.compileShader("shaders/flat_vert.glsl", "shaders/flat_frag.glsl");
...
void SceneParticles::render()
{
    ...
    view = glm::lookAt(glm::vec3(3.0f * cos(angle), 1.5f, 3.0f * sin(angle)), vec3(0.0f, 1.5f, 0.0f), vec3(0.0f, 1.0f, 0.0f));
    model = mat4(1.0f);

    flatProg.use();
    mat4 mv = view * model;
    flatProg.setUniform("MV", mv);
    flatProg.setUniform("Proj", projection);
...
void SceneParticles::onMouseMove(double xpos, double ypos)
{
}

void SceneParticles::onMouseScroll(double xoffset, double yoffset)
{
}

void SceneParticles::onMouseButton(int button, int action)
{
}

void SceneParticles::onKey(int key, int action)
{
}

Camera* SceneParticles::getCamera()
{
    return nullptr;
}
