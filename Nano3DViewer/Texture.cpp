...
GLuint Texture::loadTexture(const std::string& name)
{
    int width, height;
    unsigned char* data = Texture::loadPixels(name, width, height);
    GLuint tex = 0;
    if (data != nullptr) {
        glGenTextures(1, &tex);
...
unsigned char* Texture::loadPixels(const std::string& fName, int& w, int& h, bool flip)
{
    int bytesPerPix;
    stbi_set_flip_vertically_on_load(flip);
    unsigned char* data = stbi_load(fName.c_str(), &w, &h, &bytesPerPix, 4);
    return data;
}
